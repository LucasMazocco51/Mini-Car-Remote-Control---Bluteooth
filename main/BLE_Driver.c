...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
        	...
        	// Copiar os 4 bytes recebidos para a variável inteira (ajustar isso com base no tamanho do dado enviado)
        	memcpy(&received_value, param->write.value, sizeof(received_value));

        	split_uint16_t result;
        	result.low_byte = received_value & 0xFF;          // Obtem os 8 bits menos significativos
        	result.high_byte = (received_value >> 8) & 0xFF;  // Desloca 8 bits para a direita e obtém os próximos 8 bits

        	ESP_LOGE(EXAMPLE_TAG, "(2) ***** byteArray(int): (%d) ***** \n",received_value);
        	ESP_LOGE(EXAMPLE_TAG, "(2) ***** byteArray(hex): (%X) ***** \n",received_value);

        	ESP_LOGE(EXAMPLE_TAG, "(2) ***** byte 1(hex): (%X) ***** \n",result.low_byte);
        	ESP_LOGE(EXAMPLE_TAG, "(2) ***** byte 2(hex): (%X) ***** \n",result.high_byte);
