...
#define SAMPLE_DEVICE_NAME          "Mini Remote Car"
...
handle_uuid_map_t uuid_map[MAX_CHARACTERISTICS];
...
static const uint16_t DATA_LONG_WR	               = 0xFF01;
...
static const uint8_t dataLongWR_name[]  = "Data_Long_WR";
static const uint8_t char_value[4] = {0x11, 0x22, 0x33, 0x44};


/* Full Database Description - Used to add attributes into the database */
//Configura cada parte da comunicação BLE
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    [IDX_CHAR_VAL_A] =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&DATA_LONG_WR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE | ESP_GATT_PERM_READ_ENC_MITM,
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_user_description, ESP_GATT_PERM_READ,
      sizeof(dataLongWR_name), sizeof(dataLongWR_name), (uint8_t *)dataLongWR_name}},

};

//mostra os dispositivos pareados ao ble
...
//apaga todos dispositivos pareadosao ble
...
esp_bt_uuid_t *get_uuid_from_handle(uint16_t handle) {
    ...
    for (int i = 0; i < uuid_map_count; i++) {
        if (uuid_map[i].handle == handle) {
            return &uuid_map[i].uuid;
        }
    }
    return NULL; // Retorna NULL se não encontrar
}

//lida com os eventos do BLE
...
//trabalha com o evento de preparação de write do BLE
...
//trata com a parte de execução da escrita
    ...
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
			...
			case ESP_GATTS_WRITE_EVT:
				if(param->write.handle == gatt_db_handle_table[IDX_CHAR_VAL_A]) {
					// Capturando o comando relevante
					if (param->write.len >= 8) { // Verifica se o tamanho dos dados é suficiente
						QueueMessage message;
						message.type = TYPE_A; // Supondo que TYPE_A é um tipo genérico ou específico
						message.directionStatus = param->write.value[6];
						message.speedStatus = param->write.value[5];
						...
						//printf("\n");
						// Enviar para a fila
						if (xQueueSend(receivedValueQueue, &message, portMAX_DELAY) != pdPASS) {
							printf("Erro ao escrever na fila\n");
						}
					} else {
						printf("Dados recebidos insuficientes para extrair o comando\n");
					}
				}
        ...
        case ESP_GATTS_DISCONNECT_EVT:
            ...
            if (xQueueSend(disconnectHandleQueue, &disconnected, portMAX_DELAY) != pdPASS)
            {
            	printf(" Error in QUEUE DISCONNECT EVENT");
            }
            esp_ble_gap_start_advertising(&adv_params);
            break;
        ...
        case ESP_GATTS_ADD_CHAR_EVT:
        	printf("Entrei no GATTS_ADD_CHAR EVT\n");
                   if (uuid_map_count < MAX_CHARACTERISTICS) {
                       uuid_map[uuid_map_count].handle = param->add_char.attr_handle;
                       uuid_map[uuid_map_count].uuid = param->add_char.char_uuid;
                       uuid_map_count++;
                   }
            break;
...
void app_BLE(void)
{
    ...
    ESP_LOGE(EXAMPLE_TAG, " enable controller failed");
