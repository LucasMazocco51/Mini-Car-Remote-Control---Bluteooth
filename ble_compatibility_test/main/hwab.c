...
void app_loop(void *pvParameters)
{
	...
	while(1){
		if(xSemaphoreTake(dataReadySemaphoreModelStep, portMAX_DELAY)==pdTRUE)
		{
			...
			mini_car_ble_step();
			//esp_task_wdt_reset();
			printf("IN1 : %f\n", mini_car_ble_Y.IN1_RMotor);
			printf("IN2: %f\n", mini_car_ble_Y.IN2_RMotor);
			printf("IN3 : %f\n", mini_car_ble_Y.IN3_LMotor);
			printf("IN4 : %f\n", mini_car_ble_Y.IN4_LMotor);
			printf("PWM R : %f\n", mini_car_ble_Y.Drive_RMotor);
			printf("PWM L : %f\n", mini_car_ble_Y.Drive_LMotor);
...
void SWC_Start_Motor(void *pvParameters)
{
	while(1)
	{
		// Atualizar os GPIOs de direção do Motor Direito
        gpio_set_level(IN_1RMotor, mini_car_ble_Y.IN1_RMotor); // Exemplo: 1 para HIGH, 0 para LOW
        gpio_set_level(IN_2RMotor, mini_car_ble_Y.IN2_RMotor); // Exemplo: 1 para HIGH, 0 para LOW

        // Atualizar os GPIOs de direção do Motor Esquerdo
        gpio_set_level(IN_3LMotor, mini_car_ble_Y.IN3_LMotor); // Exemplo: 1 para HIGH, 0 para LOW
        gpio_set_level(IN_4LMotor, mini_car_ble_Y.IN4_LMotor); // Exemplo: 1 para HIGH, 0 para LOW
